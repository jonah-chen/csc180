...
for word in text:
    try:
        word_counts[word] += 1
    except KeyError: # If the word is not already an key in the dictionary, it returns a key error. This will catch the error and add a the word as a new key.
        word_counts[word] = 1
...
def get_yahoo_search_results(term):
    term = term.replace(" ", "+")
    page = urlopen(f"https://ca.search.yahoo.com/search?p={term}&fr=yfp-t&fp=1&toggle=1&cop=mss&ei=UTF-8").read().decode("utf-8")
    index = page.find("0 result")
    if index < 0:
        return 0
...
if __name__ == "__main__":
    words = ["I", "am", "a", "sick", "man.", "I", "am", "a", "spiteful", "man.", "I", "am","an", "unattractive", "man.", "I", "believe", "my", "liver", "is", "diseased.","However,", "I", "know", "nothing", "at", "all", "about", "my", "disease,", "and","do", "not", "know", "for", "certain", "what", "ails", "me."]
    with ThreadPoolExecutor(max_workers=12) as executor:
        results = executor.map(get_yahoo_search_results, words)
        print(list(results))
